...
find 3–7 nouns from WordNet that best emobdy the concepts.
...
# for when I switch it to fetching synsetIDs (will take looking up the words)
recommend_synsets_pre_prompt2 = """I want you to look at an apodosis and
find 3–7 noun synsets from WordNet that best emobdy the concepts.
Considering this is divinatory, avoid literal interpretations if possible,
erring on the side of polysemy and applicability to a divinatory reading.
Provide a Python-formatted, single-quoted, comma-separated list of synsetIDs from Wordnet.
An example is "['cat.n.01', 'dog.n.01', 'bird.n.01']" for "pets", though this is just an example.
Do not provide an explanation or any other text than the list response.
The prior text are instructions.
The apodosis I want you to evaluate follows.
"""
...
class AskChatGPT():
    ...
    def recommend_synsets(self, apodosis_to_exampine):
        full_synset_prompt = recommend_synsets_pre_prompt + sentence # prepending the control prompt
        
        parse_attempts = 3
        while True:
            attempt_count -= 1
            chatgpt_response = gpt_request(full_wordnet_prompt) # getting chatGPT's starting guesses
